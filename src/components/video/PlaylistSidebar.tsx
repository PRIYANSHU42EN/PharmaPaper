...
export default function PlaylistSidebar({ currentVideoId, playlistName, videos }: PlaylistSidebarProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#07080f]/80 backdrop-blur-md rounded-2xl border border-[#888888]/15 overflow-hidden">
      {/* Header */}
      <div className="p-4 bg-white/5 border-b border-[#888888]/10 flex flex-col gap-1">
        <span className="text-[9px] uppercase font-mono tracking-widest text-[#888888]">Playlist Context</span>
      ...
      {/* Videos List */}
      <div className="flex-1 overflow-y-auto divide-y divide-[#888888]/5 p-2 space-y-1 scrollbar-thin scrollbar-thumb-brand/20">
        {videos.map((vid, idx) => {
          ...
          return (
            <Link
              ...
              className={`flex items-start gap-3 p-3 rounded-xl transition-all duration-300 ${
                isActive
                  ? "bg-[#888888]/10 border border-[#888888]/20 text-brand-cream"
                  : "hover:bg-white/5 text-brand-cream/75 hover:text-brand-cream border border-transparent"
              }`}
            >
              {/* Number indicator / Play / Lock status */}
              <div className="flex-shrink-0 w-6 h-6 rounded-lg bg-black/40 border border-[#888888]/10 flex items-center justify-center text-[10px] font-mono font-bold text-brand-cream/40">
                {isActive ? (
                  <svg className="w-2.5 h-2.5 fill-brand" viewBox="0 0 24 24">
                    <path d="M8 5v14l11-7z" />
                  </svg>
                ) : vid.isPremium ? (
                  <svg className="w-2.5 h-2.5 fill-current text-[#888888]" viewBox="0 0 24 24">
